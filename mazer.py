...
class MazerBot:
    def __init__(self):
        ...
        self.screen_rect = (287, 23, 86, 320)
        self.track_rect = None
        self.screen_depth = None
        self.track_depth = None
        self.screen_init = None
        self.screen_box_ix_prev = -1
        self.track_buffer = []
        self.screen_buffer = []
        self.track_buffer_size = 20
        
    def cmap_lut_apply(self,x):
        x[:,:,0] = cv2.LUT(x,self.turbo_colormap_data[:,0])[:,:,0]
        x[:,:,1] = cv2.LUT(x,self.turbo_colormap_data[:,1])[:,:,1]
        x[:,:,2] = cv2.LUT(x,self.turbo_colormap_data[:,2])[:,:,2]
        return x
    def depth_callback(self, stereo_data):
        stereo_data = self.bridge.imgmsg_to_cv2(stereo_data, 'rgb8')
        
        if self.track_init:
           

            if self.screen_rect is not None:
                self.screen_depth = cv2.UMat(stereo_data,self.screen_rect).get()
            if self.track_rect is not None:
                self.track_depth = cv2.UMat(stereo_data,self.track_rect).get()
                self.screen_box_depth = cv2.UMat(stereo_data,self.screen_box).get()
            # self.disp_image =stereo_data
            self.disp_image =self.cmap_lut_apply(stereo_data)


            
            # cv2.imshow("depth", self.disp_image)
            # cv2.waitKey(3)
    ...
    def stereo_callback(self, stereo_data):
        ...
        if not self.screen_init :
            ...
            stereo_data, self.bbox,screen_box = track(stereo_data,self.ixs,self.rect_pts,self.bbox,self.tracker)
            self.track_rect = np.array(self.bbox, dtype=np.int)
            self.screen_box = screen_box[1]
            self.screen_box_ix = screen_box[0]
            cv2.rectangle(stereo_data, (self.track_rect[0], self.track_rect[1]),
                          (self.track_rect[0] + self.track_rect[2], self.track_rect[1] + self.track_rect[3]), (255, 0, 0),
                          2)
        ...
        if self.track_rect is not None and self.screen_depth is not None and self.track_depth is not None:
            ...
            if s < 1.5 or track_mean <= screen_mean:
                ...
                fill_matches(stereo_data,self.ixs,self.rect_pts,self.bbox,color=(0,255,0))
                if self.screen_box_ix != -1 and self.screen_box_ix_prev != self.screen_box_ix:
                    aud_str = 'mpg123 auds/{}.mp3 --frames 30 --quiet &'.format(self.screen_box_ix)
                    os.system(aud_str)
